...
- **Mastra**: Read `${SKILL_DIR}/references/mastra.md` and follow the steps there.

## Step 4: Register in coral-server config

Read `~/.coral/coral-server/src/main/resources/config.toml` and add the agent's absolute path to the `local_agents` list under `[registry]`.
...
Tell the user:
- What files were created and what they do
- That the agent is now registered with coral-server
- To start (or restart) coral-server with: `cd ~/.coral/coral-server && ./gradlew run`
- The agent will be auto-launched by coral-server when a session requests it
- They can still run their agent standalone (e.g., `npm run dev` for Mastra) without coral - the coral integration is additive and doesn't break local development
